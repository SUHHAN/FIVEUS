...
public class NpcScript : MonoBehaviour
{
    ...
    public void OnTalkButtonClick()
    {
        ...
        dialogueManager.currentDialogueIndex = dialogueManager.dialogueEntry.FindIndex(d => d.id == 1); // id가 1인 대화 찾기
        dialogueManager.PrintDialogueEntry(dialogueManager.currentDialogueIndex); // 대화 시작
        isTalking = true; // 대화 상태 설정
    }

    public void ShowChoices(string choice1, string choice2, Action choice1Action, Action choice2Action)
    {
        choice1Button.gameObject.SetActive(!string.IsNullOrEmpty(choice1));
        choice2Button.gameObject.SetActive(!string.IsNullOrEmpty(choice2));

        if (!string.IsNullOrEmpty(choice1))
        {
            choice1Button.GetComponentInChildren<TextMeshProUGUI>().text = choice1;
            choice1Button.onClick.RemoveAllListeners();
            choice1Button.onClick.AddListener(() => choice1Action.Invoke());
        }
        ...
        if (!string.IsNullOrEmpty(choice2))
        {
            ...
            choice2Button.onClick.AddListener(() => choice2Action.Invoke());
        }
    }
